...
mkdir -p .publish
echo $apiPublishProfile > .publish/api_publish_profile.json

# Set API App publish profile as GitHub secret
gh secret set AZURE_WEBAPP_PUBLISH_PROFILE_API -b"$(cat .publish/api_publish_profile.json)"
...
gh secret set AZURE_WEBAPP_PUBLISH_PROFILE_WEB -b"$(cat .publish/web_publish_profile.json)"
